...
class Transformer(Module):
    ...
    def __init__(self, d_model: int = 512, nhead: int = 8, num_encoder_layers: int = 6, b=0.001,
...
class TransformerEncoder(Module):
    ...
    def __init__(self, encoder_layer, num_layers, norm=None, b=0.001):
    ...
    def forward(self, src: Tensor, mask: Optional[Tensor] = None, types: Optional[dict] = None,
        ...
        """
        output = src
        for (current_head, mod) in enumerate(self.layers):
            output = mod(output, types=types, src_mask=mask, src_key_padding_mask=src_key_padding_mask, current_head=current_head)
...
class TransformerEncoderLayer(Module):
    ...
    def forward(self, src: Tensor, types: Optional[dict] = None, src_mask: Optional[Tensor] = None,
                src_key_padding_mask: Optional[Tensor] = None, current_head=None) -> Tensor:
        ...
        if self.prenorm:
            ...
            src2 = self.self_attn(src, src, src, types=types, attn_mask=src_mask,
                      key_padding_mask=src_key_padding_mask, current_head=current_head)[0]
...
class TransformerDecoder(Module):
    ...
    def __init__(self, decoder_layer, num_layers, norm=None, b=0.001):
    ...
    def forward(self, tgt: Tensor, memory: Tensor, types: Optional[dict] = None, tgt_mask: Optional[Tensor] = None,
        ...
        for (current_head, mod) in enumerate(self.layers):
            output = mod(output, memory, types=types, tgt_mask=tgt_mask,
                         ...
                         tgt_key_padding_mask=tgt_key_padding_mask,
                         memory_key_padding_mask=memory_key_padding_mask, current_head=current_head)
...
class TransformerDecoderLayer(Module):
    ...
    def forward(self, tgt: Tensor, memory: Tensor, tgt_mask: Optional[Tensor] = None,
                ...
                tgt_key_padding_mask: Optional[Tensor] = None,
                memory_key_padding_mask: Optional[Tensor] = None, current_head=None) -> Tensor:
...
class MultiheadAttention(torch.nn.Module):
    def __init__(self, embed_dim, num_heads, dropout=0., bias=True, add_bias_kv=False, add_zero_attn=False, kdim=None,
                 vdim=None, share_qk=False, b=0.001):
    ...
    def forward(self, query, key, value, types=None, key_padding_mask=None, need_weights=True, attn_mask=None, current_head=None):
        if attn_mask is not None:
            if attn_mask.dim() == 2:
        ...
        if (current_head!=0):
          self.alpha_c = (-1)*math.log(self.b/(self.num_heads-1) * current_head)

        self.multihead_attention = MultiheadAttentionContainer(self.num_heads,
                                                               self.in_proj_container,
                                                               LaSAttention(self.embed_dim, alpha_c=self.alpha_c, kernel_size=5),
                                                               self.out_proj)
        return self.multihead_attention(query, key, value, types, attn_mask, self.bias_k, self.bias_v, current_head=None)
...
class LaSAttention(torch.nn.Module):
    ...
    def forward(self, q, k, v, mask=None):
        ...
        d_l = get_relative_positions(seq_len=seq_len).to(self.device)

        eld_matrix = (self.alpha_c*(-1)*d_l).to(self.device)
